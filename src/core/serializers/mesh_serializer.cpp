...
using vec3 = xb2at::core::mesh::vector3;
...
namespace core {
	...
	void meshSerializer::Serialize(mesh::mesh& meshToDump, const meshSerializerOptions& options) {
		...
		} else if (options.OutputFormat == meshSerializerOptions::Format::GLTFText) {
		...
		if (options.OutputFormat == meshSerializerOptions::Format::GLTFBinary || options.OutputFormat == meshSerializerOptions::Format::GLTFText) {
			// do gltf things

			fx::gltf::Document doc{};
			doc.asset.generator = "XB2AT";
			doc.asset.version = "replaceme";
			...
			fx::gltf::Save(doc, ofs, outPath.filename().string(), false);
